...
Analyzing and extracting Finnish personal identity numbers is rather straightforward even with a naked eye. However, hetu-package is best suited for handling large number of PINs, which would be cumbersome by hand.

Hetu-package can extract the following information from hundreds of thousands of PINs:
...
Loading the package and setting a few imaginary example PINs:
...
Hetu-function is the backbone of the package and lots of information can be extracted in data frame format:

```{r, eval = TRUE}
knitr::kable(hetu(example_pins))
```

The information in the data frame can be extracted with extract-parameter or, for the convenience of the end user, by using specialized functions:
...
Extracting age works only with a specialized function. In this example we will also introduce the ability to generate random PINs with rhetu-function. All PINs are meant only for temporary use:
...
boxplot(hetu_age(example_pins2))
```
...
As in sweidnumbr, hetu-package has two functions that can be used with Finnish Business Identity Numbers (y-tunnus). Finnish business identity numbers have the form 1234567-8, where the last number is a checknumber.[^2] The following functions are available:
...
[^2]: Yritys- ja yhteisötietojärjestelmä. Y-tunnus. URL: https://www.ytj.fi/index/y-tunnus.html
